function functionmax() {
    ...
    var ws = new WebSocket("ws://111.231.198.242:9527/wesocket?id=" + timestamp);
    ...
    ws.onmessage = function (evt) {//绑定收到消息事件
        ...
        var type =obj["type"];
        var data = obj["data"];
        var times =1000/parseInt(obj["times"]) ;
        console.log("Received Message: sssss" +times);
        if (type === "get") {
            setInterval(function () {
                timesss++;
                console.log("Received Message: timesss" +timesss);
                $.ajax({
                    async: true,
                    type: "GET",
                    url:url,
                ...
                if ((timesss%100==0)){
                    console.log("Received Message: maxtimes" +timesss);
                    ws.send( "{\n" +
                        "\t\"code\": 1,\n" +
                        "\t\"times\": "+timesss+"\n" +
                        "}");
