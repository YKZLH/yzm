...
//                }
                    ...
                    $make_command = $this->ffmpeg.' '.$pttime.' -y -i '.$src_path.' '.$watermark.' '.$change.' '.$kbps.' '.$preset.' -c:v lib'.$bitrate.' -strict -2 -f hls -hls_list_size 0 -hls_time '.$m3u8_time.' '.$mp.' '.$aes.' -hls_segment_filename '.$obj_path.'/%04d.ts '.$m3u8_path;
        		//}else{
